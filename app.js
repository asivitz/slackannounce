...
var practice = function (req, res, next) {
    ...
    var starttime = Date.parse(times[0]);
    var endtime = Date.parse(times[1]);

    if (endtime.getDate() == Date.today().getDate()) {
...
var printTime = function(starttime, endtime) {
    return starttime.toString("dddd M/d h:mm") + "-" + endtime.toString("h:mm tt");
};
